module ga_unit
    use util, only: filled, integers, mask_index, safe_inv, sort
    use interface
    implicit none
    ...
    function rank_pareto_base(val, mode) result(rank)
        ...
        if (present(mode) .and. mode > 0) then
            rank = rank_pareto_core([((all(val(:, i) > val(:, j)), j = 1, n), &
                                      i = 1, n)], n)
        else
            rank = rank_pareto_core([((all(val(:, i) >= val(:, j)) .and.       &
                                       any(val(:, i) /= val(:, j)), j = 1, n), &
                                      i = 1, n)], n)
        end if
    ...
    function rank_pareto_proc(val, proc) result(rank)
        ...
        n = size(val, dim=2)
        rank = rank_pareto_core([((proc(val(:, i), val(:, j)), j = 1, n), &
                                  i = 1, n)], n)
    ...
    function rank_pareto_core(dominated, n) result(rank)
        ...
        do rank_no = 1, n
            ...
            do i = 1, n
                ...
                if (rank(i) == 0) then
                    num_dominated(i) = num_dominated(i) - &
