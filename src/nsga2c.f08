    ...
    subroutine save_result_elite_feasible(this, filename, elite, feasible)
        ...
        open(newunit=unit, file=filename)
            ...
            do i = 1, this%pop_size
                if ((elite == "all" .or.                                      &
                     xor(elite == "only", this%population(i)%rank > 1)) .and. &
                    (feasible == "all" .or.                                   &
                     xor(feasible == "only", .not. this%population(i)%indiv%feasible))) then
        ...
        open(newunit=unit, file=filename)
            ...
            outer: do j = 1, size(this%history, dim=2)
                inner: do i = 1, this%pop_size
                    ...
                    if ((elite == "all" .or.                                      &
                         xor(elite == "only", this%history(i, j)%rank > 1)) .and. &
                        (feasible == "all" .or.                                   &
                         xor(feasible == "only", .not. this%history(i, j)%indiv%feasible))) then
