...
type :: TPopulation
    integer :: i_gen
    type(TIndividual), allocatable :: indiv(:)
...
program main
  ...
  integer :: n_x, n_f, n_g, n_pop, n_gen
  character(32) :: tf, ts, tc, tm
  real(8), allocatable :: lbounds(:), ubounds(:)
  integer :: unit, start_time, i

  type(indiv), allocatable :: population(:)
  ...
  open(newunit=unit, file="inputopt.txt", status="old")
    ...
    read(unit, *) n_pop      ! ... population size
